...
export default function About(): ReactElement {
  const { title, subtitle, bodyText }: BasePageTextProps =
    pageContent.basePageText;
  ...
  return (
    <div className="relative flex flex-col sm:mx-auto">
      {/* BASE PAGE TEXT */}
      <BasePageText title={title} subtitle={subtitle} bodyText={bodyText} />

      {/* PROF. EXPERIENCE SECTION */}
      <div
        className={
          "bg-gradient-to-r from-slate-900 to-slate-800 section-shadow mt-3"
        }
      >
        <MotionSlider
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-0 justify-items-center mb-0 sm:mb-4 items-center px-6 sm:px-0">
            {workExperienceCards.map(
              (button: any, index: number): ReactElement => {
                return (
                  <AboutPageButton
                    key={index}
